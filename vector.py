...
def zero(dimV: int, dimW: Optional[int] = None):
...
def crossProduct(v, w):
    ...
    # cross products only work in 3D. boo
    if v.dim != 3:
        raise ValueError("cross product can only be calculated in three dimensions")
    if v.dim != w.dim:
        raise ValueError("vectors must share dimension")
    ...
    product = []
    for i in range(3):
        a = (v[dim[i]] * w[dim[i+1]]) - (v[dim[i+1]] * w[dim[i]])
        product.append(copy.deepcopy(a))
    
    return Vector(product)

def maxLength(l: list) -> int:
...
class Vector:
    ...
    def proj(self, other, Gij = None):
        "Returns the projection of a vector onto another vector."
        # must work with Vectors and NormalVectors!!
        # self.proj(other) returns proj self(other) aka projection of OTHER ONTO SELF
        if Gij is None:
            Gij = identity(self.dim)
        ...
        # this is just the easiest way for me to get the simplified mag2 honestly
        mag2 = self.mag2(Gij)
        c = Rational(innerProduct(self, other, Gij), mag2)
        c = c.simplify()

        projection = c * self
        for i in range(self.dim):
            if isinstance(projection[i], Rational):
                projection[i] = projection[i].simplify()
        return projection

    def normal(self, Gij = None):
...
# Always define a Vector first, then normal() it.
class NormalVector:

    # MATH NEEDS TO BE ABLE TO FUNCTION WITH BOTH OTHER NORMALVECTORS AND VECTORS

    def __init__(self, divSqrt: int, vector: list):
    ...
    # these two functions exist to allow mag2() and mag() when the input might be either
    # a Vector or NormalizedVector
    def mag2(self, Gij = None) -> int:
        "Returns the magnitude squared of a normalized vector. Exists to allow mag2() when a Vector is expected."
        return 1

    def mag(self, Gij = None) -> int:
        "Returns the magnitude of a normalized vector. Exists to allow mag() when a Vector is expected."
        # lol
        return 1
    
    def normal(self, Gij = None):
        "Returns itself. Exists to allow normal() when a Vector is expected."
        return self
...
class Matrix:
    ...
    def rowVectors(self) -> list[Vector]:
    ...
    def columnVectors(self) -> list[Vector]:
